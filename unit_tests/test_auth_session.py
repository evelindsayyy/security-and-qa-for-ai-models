...
class TestAuthSession(unittest.TestCase):
    ...
    def test_public_view_default(self):
        with self.app.test_request_context("/"):
            from auth.session import current_user, effective_user, get_view_mode

            self.assertEqual(get_view_mode(), "public")
            self.assertIsNone(current_user())
            self.assertIsNone(effective_user())

    def test_dev_login_required_for_private_mode(self):
    ...
    def test_dev_login_then_private_view(self):
        rv = self.client.get("/auth/login")
        self.assertEqual(rv.status_code, 302)
        rv = self.client.post("/auth/view-mode", data={"mode": "private"})
        self.assertEqual(rv.status_code, 302)
        rv = self.client.get("/auth/me")
        data = rv.get_json()
    ...
    def test_private_view_requires_login_after_logout(self):
        self.client.get("/auth/login")
    ...
    def test_allowlist_when_auth_disabled(self):
        with self.app.test_request_context("/"):
            from auth.session import dev_user_from_env, is_allowlisted

            self.assertTrue(is_allowlisted(dev_user_from_env()))
